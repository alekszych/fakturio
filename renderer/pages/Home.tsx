	...
	return (
		<div className={"absolute left-2 right-2 top-2 bottom-2 m-auto h-fit w-4/5 max-w-2xl p-5 pb-10"}>
			...
			<div className={"flex mt-4 justify-center flex-wrap"}>
				<Button onClick={() => router.push("/AddAccount")}> <BsPersonFillAdd className={"mr-2 text-xl"}/> <p className={"text-sm py-1"}> Dodaj nowe konto </p> </Button>
				<Button onClick={() => router.push("/DeleteAccount")}> <BsPersonFillDash className={"mr-2 text-xl"}/> <p className={"text-sm py-1"}> Usuń konto </p> </Button>
				{account && <Button onClick={handleLogin}> <FiLogIn className={"mr-2 text-xl"}/> <p className={"text-sm py-1"}> Zaloguj się </p> </Button>}
			</div>
		</div>
	)
}
